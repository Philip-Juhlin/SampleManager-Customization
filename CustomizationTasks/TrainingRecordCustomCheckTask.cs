...
namespace Customization.Tasks
{
    ...
    /// </summary>
    [SampleManagerTask("TrainingRecordCustomCheck")]
    public class TrainingRecordCustomCheckTask : SampleManagerTask, IBackgroundTask
    {
        ...
        private readonly IList<PersonnelTraining> OperatorsNeedTraining = new List<PersonnelTraining>();
        ...
        private void CheckTrainedOperators(TrainingCourseBase trainingCourse, ref StringBuilder mailMessage)
        {
            ...
            foreach (PersonnelTraining persTraining in persTrainings)
            {
                ...
                else if (Math.Abs(trainingCourse.RetestInterval.TotalMilliseconds) > 0.4)
                {
                    NullableDateTime warningDate = persTraining.RetestDate.Value.Subtract(trainingCourse.RetestWarningPeriodInterval);

                 
                    NullableDateTime lastAlertDate = persTraining.RetestDate.Value.AddMonths(1);

                   

                    
                    if (persTraining.RetestDate <= Library.Environment.ClientNow && Library.Environment.ClientNow <= lastAlertDate)
                    { 
                    idLength = persTraining.ParentPersonnel.Identity.Length > idLength
                                    ? persTraining.ParentPersonnel.Identity.Length
                                    : idLength;
                    retestNow.Add(persTraining);
                        OperatorsNeedTraining.Add(persTraining);
                    

                    }
                    else if (warningDate <= Library.Environment.ClientNow && Library.Environment.ClientNow <= lastAlertDate)
                    {
                    idLength = persTraining.ParentPersonnel.Identity.Length > idLength
                                    ? persTraining.ParentPersonnel.Identity.Length
                                    : idLength;
        ...
        private void MailOperator()
        {

            foreach(PersonnelTraining trainie in OperatorsNeedTraining)
            {
                if (trainie.ParentPersonnel.Email != null)
                {
                    StringBuilder mailBody = new StringBuilder();
                    mailBody.Append(trainie.TrainingCourse.Name);
                    mailBody.Append(' ');
                    mailBody.Append($"Needs to be retrained before {trainie.RetestGraceDate.Value}");
                    string messageText = mailBody.ToString();
                    MailToUser((Personnel)trainie.ParentPersonnel, messageText);
                }
        ...
        private void MailToUser(Personnel oper, string mailMessage)
        {
            ...
            catch (Exception e)
            {
                ...
                {
                    Logger.DebugFormat("Error Sending mail {0}", e.Message);
                }
            }
        }
